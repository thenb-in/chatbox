...
(function () {
  ...
  // Wait briefly for NTVisitorProfile in case it's loading in parallel, but
  // give up so this module also works on hosts that have no profile system
  // (e.g. personal sites that only need the dial-picker dropdown).
  function boot(attemptsLeft) {
    if (typeof attemptsLeft !== 'number') attemptsLeft = 20;
    if (!window.NTVisitorProfile) {
      if (attemptsLeft <= 0) return; // no profile on this host — skip IP fetch entirely
  ...
  // ISO-3166 alpha-2 → E.164 country calling code. Used to prefill the
  // chatbot phone input with the visitor's likely dial code so they don't
  // have to type it themselves, and to surface the dial code in the lead
  // email even if the visitor strips the prefix.
  var DIAL_CODES = {
  ...
  // Cached so the chatbot's country-code dropdown doesn't rebuild the list
  // on every re-render of the phone step.
  var _allDialCodes = null;

  /**
   * Sorted [{ iso, dial, name }, ...] for populating a country-code picker.
   * Names come from Intl.DisplayNames where available; older browsers fall
   * back to the ISO code so the option is still distinguishable.
   */
  function getAllDialCodes() {
    ...
    var list = Object.keys(DIAL_CODES).map(function (iso) {
      var name = iso;
      if (names) {
        try { name = names.of(iso) || iso; } catch (e) {}
      }
  ...
  window.NTIPLocation = {
    ...
    get: getStored,
    /** E.164 dial code (e.g. "+91") for the visitor's IP country, or null. */
    getDialCode: getDialCode,
    /** ISO-2 country code for the visitor's IP, or null. */
    getCountryCode: function () {
      var loc = getStored();
      return (loc && loc.country_code) ? String(loc.country_code).toUpperCase() : null;
    },
    /** Sorted list of every supported country with dial code + name. */
